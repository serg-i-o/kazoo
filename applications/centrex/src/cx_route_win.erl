...
execute_callflow(JObj, Call) ->
    io:format("\n~p.execute_callflow/2:\nCallJObj=~p\n",[?MODULE, kapps_call:to_json(Call)]),
    io:format("setting initial information about the call\n"),
...
%%    io:format("\n~p.should_restrict_call/1:\nCallId=~p\nDefaultEndpointId=~p\nEndpointId=~p\n",
%%        [?MODULE,kz_doc:id(Call),DefaultEndpointId,EndpointId]),
