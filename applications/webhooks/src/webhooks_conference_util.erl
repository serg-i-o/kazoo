...
handle_event(JObj, Props) ->
    ...
    HookEvent = erlang:binary_to_atom(kz_json:get_value(<<"Event-Name">>, JObj), 'utf8'),
    io:format("EventName=~p\n",[HookEvent]),
    case HookEvent of
        'event'   -> handle_conf_event(JObj, Props);
        'command' -> handle_command_event(JObj, Props);
        _ -> lager:debug("failed to determine event-name")
    end.

handle_conf_event(JObj, _Props) ->
    EventName = kz_json:get_value(<<"Event">>, JObj),
    case kz_hooks_util:lookup_account_id(JObj) of
        {'error', _R} ->
            lager:debug("failed to determine account id for ~s", [EventName]);
        {'ok', AccountId} ->
            J = kz_json:set_value([<<"Custom-Channel-Vars">>
                ,<<"Account-ID">>
            ], AccountId, JObj),
            maybe_handle_conf_event(AccountId, EventName, J)
    end.

-spec handle_command_event(kz_json:object(), kz_term:proplist()) -> any().
handle_command_event(JObj, Props) ->
    handle_command_event(JObj, Props, kz_json:get_value(<<"Conference-ID">>, JObj)).
handle_command_event(_JObj, _Props, 'undefined') ->
    lager:debug("failed to determine conference id in the event");
handle_command_event(JObj, Props, ConferenceId) ->
    handle_command_event(JObj, Props, ConferenceId, kz_json:get_value(<<"Application-Name">>, JObj)).
handle_command_event(_JObj, _Props, _ConferenceId, 'undefined') ->
    lager:debug("failed to determine command in the event");
handle_command_event(JObj, _Props, _ConferenceId, ConfCommand) ->
    case kz_hooks_util:lookup_account_id(JObj) of
        {'error', _R} ->
            lager:debug("failed to determine account id for");
        {'ok', ConfCommand} ->
            J = kz_json:set_value([<<"Custom-Channel-Vars">>
                ,<<"Account-ID">>
            ], ConferenceId, JObj),
...
maybe_handle_conf_event(AccountId, HookEvent, _JObj) ->
